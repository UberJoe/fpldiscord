...
- [ ] `LeagueTransactions(gw)` resolves raw `type`/`status` codes to
      `waiver`/`freeAgent` and `accepted`/`failed`, exposes `priority`, and excludes
      trades
- [ ] `/waivers` with no args shows the latest processed GW's accepted claims; `[gw]`
      selects a GW; `result` = `accepted` (default) / `failed` / `all`
- [ ] `failed`/`all` output shows who out-bid whom in bid order; long output splits
      across multiple messages with no truncation
- [ ] `GET /api/waivers?gw=N` returns `gw` + `rows[]` (`ownerName`, `entryId`, `in`,
      ...
      echoed in `data.gw`
- [ ] `priority` is present on the unauthenticated feed so bid order works without
      login
- [ ] The web view has a GW selector defaulting to the latest processed GW, one table
